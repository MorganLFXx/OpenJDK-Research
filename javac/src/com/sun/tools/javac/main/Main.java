...
public class Main
{
    ...
    { // XXX sb protected
        ...
        while (ac < flags.length)
        {
            ...
            if (option.hasArg())
            {
                ...
                ac++;
                if (option.process(optionHelper, flag, operand)) return null;
            } else
            {
                if (option.process(optionHelper, flag)) return null;
            }
        }
     ...
     * @param args The command line parameters.
     */
    public Result compile(String[] args, Context context, List<JavaFileObject> fileObjects,
                          Iterable<? extends Processor> processors)
    {
        return compile(args, null, context, fileObjects, processors);
    }

    public Result compile(String[] args, String[] classNames, Context context, List<JavaFileObject> fileObjects,
                          Iterable<? extends Processor> processors)
    {
        ...
        try
        {
                ...
                while (iter.hasNext())
                {
                    ...
                    for (List<String> p : pluginsToCall)
                    {
                        if (plugin.getName().equals(p.head))
                        {
                            ...
                            try
                            {
                                if (task == null) task = JavacTask.instance(pEnv);
                                plugin.init(task, p.tail.toArray(new String[p.tail.size()]));
                            } catch (Throwable ex)
                            {
                                if (apiMode) throw new RuntimeException(ex);
                                pluginMessage(ex);
                                return Result.SYSERR;
                            }
                        }
            ...
            if (xdoclint != null || xdoclintCustom != null)
            {
                ...
                if (!(doclintOpts.size() == 1 && doclintOpts.iterator()
                                                            .next()
                                                            .equals(DocLint.XMSGS_CUSTOM_PREFIX + "none")))
                {
            ...
            if (comp.errorCount() != 0) return Result.ERROR;
    ...
    void showClass(String className)
    {
        ...
        if (url == null) pw.println("  class not found");
        else
        {
            pw.println("  " + url);
            try
